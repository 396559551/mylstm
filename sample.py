...
def main():
    ...
    # Observed length of the trajectory parameter轨迹参数的观测长度
    parser.add_argument('--obs_length', type=int, default=8,
                        help='Observed length of the trajectory')
    # Predicted length of the trajectory parameter轨迹参数的预测长度
    ...
    # Parse the parameters解析参数
    sample_args = parser.parse_args()

    # Save directory
    save_directory = 'save_social_model/' + str(sample_args.test_dataset) + '/'

    # Define the path for the config file for saved args为保存的参数定义配置文件的路径
    ...
    for batch in range(dataloader.num_batches):
        ...
        # Get nodes and nodesPresent获取节点和存在的节点
        ...
        # Extract the observed part of the trajectories提取轨迹的观察部分
...
def sample(nodes, nodesPresent, grid, args, net, true_nodes, true_nodesPresent, true_grid, saved_args, dimensions):
    ...
    # Initialize the return data structure初始化返回数据结构
    ...
    for tstep in range(args.obs_length-1, args.pred_length + args.obs_length - 1):
        ...
        # Extract the mean, std and corr of the bivariate Gaussian提取
        mux, muy, sx, sy, corr = getCoef(outputs)
        # Sample from the bivariate Gaussian来自二元高斯的样本
        ...
        # List of nodes at the last time-step (assuming they exist until the end)最后一个时间步骤的节点列表（假设它们一直存在到最后
