...
class DataLoader():
    ...
    def __init__(self, batch_size=48, seq_length=1, datasets=[0, 1, 2, 3, 4], forcePreProcess=False, infer=False):
        ...
        forcePreProcess : Flag to forcefully preprocess the data again from csv files标记以强制再次从csv文件中预处理数据
        '''
    ...
    def frame_preprocess(self, data_dirs, data_file):
        ...
        该函数会将每个数据集的pixel_pos.csv文件预处理为具有可使用的占用网格的数据
        params:
        ...
        valid_frame_data = []
        # frameList_data would be a list of lists corresponding to each dataset frameList_data将是与每个数据集相对应的列表的列表
    ...
    def next_batch(self, randomUpdate=True):
        '''
        Function to get the next batch of points获取下一批点的功能
        '''
        # Source data
        x_batch = []
        # Target data
    ...
    def tick_batch_pointer(self, valid=False):
        '''
        Advance the dataset pointer前进数据集指针
        '''
        if not valid:
